...
" PsyC keywords
syn keyword psycStatement application agent with global body advance before send receive display endbody next consult firstof other endof when then sendto delayed latest jump count
...
if exists("c_comment_strings")
  ...
  if exists("c_no_comment_fold")
    ...
    " comment.
    syntax region cComment	matchgroup=cCommentStart start="/\(ZZ\)\=\*" end="\*\(ZZ\)\=/" contains=@cCommentGroup,cCommentStartError,cCommentString,cCharacter,cNumbersCom,cSpaceError,@Spell extend
  else
    syntax region cComment	matchgroup=cCommentStart start="/\(ZZ\)\=\*" end="\*\(ZZ\)\=/" contains=@cCommentGroup,cCommentStartError,cCommentString,cCharacter,cNumbersCom,cSpaceError,@Spell fold extend
  endif
else
  syn region	cCommentL	start="//" skip="\\$" end="$" keepend contains=@cCommentGroup,cSpaceError,@Spell
  if exists("c_no_comment_fold")
    syn region	cComment	matchgroup=cCommentStart start="/\(ZZ\)\=\*" end="\*\(ZZ\)\=/" contains=@cCommentGroup,cCommentStartError,cSpaceError,@Spell
  else
    syn region	cComment	matchgroup=cCommentStart start="/\(ZZ\)\=\*" end="\*\(ZZ\)\=/" contains=@cCommentGroup,cCommentStartError,cSpaceError,@Spell fold
  endif
endif
...
syn match   cType       "\<[_a-zA-Z][_0-9a-zA-Z]*_t\>"
