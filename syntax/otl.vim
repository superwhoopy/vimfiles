...
syn match otlText /^\t*|.*/ contains=@vikiHyperLinks,otlTodo,otlTagDef,otlTagRef,otlTextLeader,otlBold,otlUnderline display nextgroup=otlText
...
syn region otlBold start="\*[a-zA-Z0-9(_\[]" end="[a-zA-Z0-9.:?!)"\]]\*" contains=otlTextLeader
syn region otlBold start="\*-" end="-\*" contains=otlTextLeader contained
syn match otlUnderline /__.*__/ contained
syn match otlCode /^\t*|:.*/ contains=otlTextLeader display
...
hi def otlBold      gui=bold
hi def otlUnderline gui=underline
hi def otlCode      guibg=white guifg=darkgreen 

if &background == "dark"
" hi def otlLeadingSpaces ctermbg=darkred guibg=#500000
  hi def otlTodo ctermbg=3 guibg=yellow guifg=black
...
" hi def otlLeadingSpaces ctermbg=lightred guibg=#ffdddd
  hi def otlTodo ctermbg=3 guibg=yellow
